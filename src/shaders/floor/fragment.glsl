...
//     for (int i=0; i<3;i++){
//         sum += sin(cnoise(vec4(p.xy*scale,uTime))*amp);
//         amp *= 0.9;
//         scale *= 2.0;
//     }
...
void main() {
    ...
    // float perlinNoise = fbm(vPosition*29.0+ 6.2831,uTime);
    // vec3 wetColor = mixedColor / 1.2;
    // vec3 pattern = mix(mixedColor, wetColor, perlinNoise);
